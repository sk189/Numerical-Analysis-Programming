...
b=input('');
fprintf("Enter the maximum allowable approximate relative error\n");
emax=input('');
...
if way==1
  ...
  fprintf(outf , "Romberg Integration\n");
  fprintf(outf , "I = %f\n",I(1,1));
  fprintf(outf , "Number of Intervals : %f\n",iter);
  fprintf(outf , "Approximate relative error : %f\n",err);
end
...
if way==2
  ...
  filename = "output1.txt";
  outf = fopen (filename, "w");
  fprintf(outf , "Gauss-Legendre quadrature\n");
  fprintf(outf , "I = %f\n",Ix);
  fprintf(outf , "Number of Guass Points : %f\n",iter);
  fprintf(outf , "Approximate relative error : %f\n",err);
end