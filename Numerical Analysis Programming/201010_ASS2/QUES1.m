...
if method == 1
 ...
 fprintf(outf , "Guass Elimination(without pivoting)\nX\n");
 fprintf(outf ,'%f\n', X);
 fclose(outf);
end;
...
if method == 2
 ...
 LU decomposition by Doolittle method (without pivoting)
if method == 3
  L = eye(size);
  U = zeros(size,size);
  for j=1:1:size
...
LU decomposition by Crout method (without pivoting)
if method == 4
  U = zeros(size,size);
  L = zeros(size,size);
  for j=1:1:size
